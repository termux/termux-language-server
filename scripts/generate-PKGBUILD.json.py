...
for index, close_index in zip(indices, close_indices, strict=False):
    ...
    if len(words) > 2:
        ...
        kind = (
            _words[1].lstrip("(").rstrip(")") if len(words) > 1 else "string"
        )
        names = words + [_words[0]]
    else:
        names = [words[0].rstrip("()")]
        kind = words[1].lstrip("(").rstrip(")") if len(words) > 1 else "string"
    for name in names:
...
schemas["PKGBUILD"]["properties"]["msys2_references"]["items"]["pattern"] = (
    f"({'|'.join(names)})(|: .*)"
)
